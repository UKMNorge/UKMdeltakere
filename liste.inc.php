...
function UKMd_innslagsboks($inn,$place,$tittellose_innslag,$forestilling=false,$tekniskprove=false) {
	if(!in_array($inn->g('bt_id'),$tittellose_innslag)) { ?>
		<div class="group ikon_rediger"><?=UKMN_icoButton('pencil',16,'rediger')?></div>
		<div class="group ">
		...
		<?php } else { ?>
		<div class="group ikon_rediger_tittellose"><?=UKMN_icoButton('pencil',16,'rediger')?></div>
		<div class="group">
	...
	if(!$forestilling) {
		if($inn->editable()) {?>
			<div class="group_right ikon_meld_av permaGray"><?=UKMN_icoButton('delete',16,'meld av')?></div>
		<?php } else { ?>
			<div class="group_right ikon_locked permaGray" alt="Startet påmelding <?= date('d.m.y H:i', $inn->g('b_subscr_time'))?>" title="Startet påmelding <?= date('d.m.y H:i', $inn->g('b_subscr_time'))?>"><?=UKMN_icoButton('lock',16,'påmelding pågår')?></div>
		<?php }
	} elseif(!$tekniskprove) { ?>
