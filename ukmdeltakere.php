...
class UKMdeltakere extends Modul
{
	...
	public static function hook() {
		...
		add_action( 'wp_ajax_UKMdeltakere_ajax', ['UKMdeltakere', 'UKMdeltakere_ajax'] );
		add_action('network_admin_menu', ['UKMdeltakere', 'UKMdeltakere_network_menu']);

		if( is_admin() && get_option('pl_id') ) {

			// add_action('admin_menu', 'UKMdeltakere_menu');
			add_action('admin_menu', ['UKMdeltakere','meny'], 1);
			add_action('UKMWPDASH_shortcuts', ['UKMdeltakere', 'UKMdeltakere_dash_shortcut'], 30);
		}
	}
	...
	public static function meny() {
	    ...
	    $page = add_menu_page(
	        ...
	        'editor', 
	        'UKMdeltakere', 
	        ['UKMdeltakere', 'UKMdeltakereLayout'], 
	    ...
	    $page_personvern = add_submenu_page(
	        ...
	        ['UKMdeltakere', 'UKMdeltakere_personvern']
		);
		...
		// Dette skal kjøres når denne siden er aktiv (page_intoleranse)
		add_action(
	...
	public static function UKMdeltakere_ajax() {
		$JSON = new stdClass();
	...
	public static function UKMdeltakere_dash_shortcut( $shortcuts ) {	
	...
	// ## CREATE A MENU
	public static function UKMdeltakere_menu() {
	...
	public static function UKMdeltakereLayout() {
		$TWIGdata = [];
	...
	public static function intoleranser() {
		static::setAction('intoleranser');

		wp_enqueue_script('UKMdeltakere_script_tilrettelegging', plugin_dir_url(__FILE__) . 'javascript/tilrettelegging_deltakere.js');

		return static::renderAdmin();
	}

	public static function UKMdeltakere_network_menu() {
		$page = add_menu_page(
			'Deltakere', 
			'Deltakere', 
			'superadmin', 
	...
	public static function UKMdeltakere_network_search() {
		$TWIGdata = [];
	...
	public static function UKMdeltakere_personvern() {
		$TWIGdata = [
			'page' => $_GET['page'],
		];
